...
public class EnemySensor : MonoBehaviour
{
  ...
  public void Tick(EnemyBlackboard bb, Transform player)
  {
    bool detected = CheckDetect(player);

    bb.TargetVisible = detected;
    bb.Target = detected ? player : null;

    if (detected)
      bb.TargetDistance = Vector2.Distance(transform.position, player.position);
  }
